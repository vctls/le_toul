...
describe("Lyrics Store", () => {
  ...
  test("should parse simple lyrics into segments", () => {
    ...
    expect(lyricsStore.lyricSegments).toEqual([{ text: "Hello\n" }, { text: "World" }]);
  });
  ...
  test("should handle underscore separators", () => {
    ...
    expect(lyricsStore.lyricSegments).toEqual([{ text: "Hello_" }, { text: "World" }]);
  });
  ...
  test("should handle slash separators within words", () => {
    ...
    expect(lyricsStore.lyricSegments).toEqual([{ text: "Hel/" }, { text: "lo" }]);
  });
  ...
  test("should handle consecutive newlines", () => {
    ...
    expect(lyricsStore.lyricSegments).toEqual([{ text: "Line 1\n\n" }, { text: "Line 2" }]);
  });
  ...
  test("should handle leading newlines without crashing", () => {
    ...
    expect(lyricsStore.lyricSegments).toEqual([{ text: "Hello world" }]);
  });
