...
async function captureZoom(page, pointer) {
  ...
  const xA = Math.round(box.x + box.width * 0.3);
  const xB = Math.round(box.x + box.width * 0.7);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const meta = await el.evaluate((node) => ({
        ...
        border: node.querySelector('[part*="region-handle-right"]')
          ? getComputedStyle(node.querySelector('[part*="region-handle-right"]')).borderRightStyle
          : "none",
