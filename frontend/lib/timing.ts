...
function renderAssDocument(
  ...
  canvasWidth: number = SUBTITLE_CANVAS.width,
): string {
  ...
  const styleLines = tracks
    ...
    .join("\n");
  // The height getLineY computed the line positions against. The width is the caller's, and
  // only sets how much room a line has before it wraps and the aspect the renderer scales to.
...
export function createAssFile(
  ...
  return createSubtitles(screensWithTitle, options, optionsToFormatParams(options), canvasWidth);
}
