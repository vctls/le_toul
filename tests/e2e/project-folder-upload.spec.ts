...
async function makeProjectFolder(): Promise<string> {
    ...
    await fs.writeFile(path.join(folder, `${defaultTestConfig.artist} - ${RESTORED_TITLE} [karaoke].mp4`), '');
    return folder;
}
...
test.describe('Project Folder Upload', () => {
    ...
    test('loading a project folder puts the whole song back together', async ({page}) => {
        ...
        await expect(page.locator('[name="settings-file-upload"] .file-name')).toHaveText('settings.yaml');
        await expect(page.locator('[name="lyrics-file-upload"] .file-name')).toHaveText('lyrics.txt');
        await expect(page.locator('[name="timings-file-upload"] .file-name')).toHaveText('timings.json');
    ...
    test('a folder holding only some of the files loads just those', async ({page}) => {
        ...
        await expect(page.locator('[name="lyrics-file-upload"] .file-name')).toHaveText('lyrics.txt');
        await expect(page.locator('[name="settings-file-upload"] .file-name')).toHaveText('No file chosen');
